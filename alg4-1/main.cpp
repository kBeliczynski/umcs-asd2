...
bool find(const unsigned int &a, const unsigned int tab[], int begin, int end){
     if(a == tab[(int)(0.99+((begin+end)/2.0))])
        return true;
     if(begin == end)
        return false;
     if(a > tab[(int)(0.99+((begin+end)/2.0))])
        if(find(a,tab,(int)(0.99+((begin+end)/2.0)),end))
            return true;
        else return false;
     if(a < tab[(int)(0.99+((begin+end)/2.0))])
...
bool hasSameElement(Node * &first, Node * &second){
    if(first->allElement[first->amountElement-1] < second->allElement[0])
        return false;
...
void Union(Node * &first, Node * &second){second->father = first;}
